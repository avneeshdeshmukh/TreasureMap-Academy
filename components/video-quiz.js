...
export default function VideoQuiz({
  ...
  const fetchAttempts = async () => {
    ...
    const videoNotesRef = doc(
      firestore,
      "videoNotes",
      `${videoId}_${auth.currentUser.uid}`
    );
  ...
  const handleNextQuestion = async () => {
    ...
    if (nextIndex < currentQuiz.length) {
      ...
      if (!preview) {
        ...
        try {
          const { difficulty, ratio } = getQuizMetrics(
            currentQuiz,
            currentQuizPoints,
          ...
          let updateData = {
            ...
            [`courseProgress.${courseId}.courseCoins`]:
              increment(currentQuizPoints),
            "PLUH.QPS": QPS,
            "PLUH.DS": DS,
          ...
          if (att - 1 === 0) {
            updateData[`courseProgress.${courseId}.quizzesCompleted`] =
              increment(1);
            if (
  ...
  const handlePlayerReady = useCallback(
    (player) => {
      ...
      const handleSeeking = () => {
        ...
        const nextQuizTime = quizMarkers
          .map((m) => m.time)
          .find((time) => time > lastAllowedTimeRef.current);
    ...
    <>
      ...
      {currentQuestion && currentQuestion.type === "mcq" && (
        <MCQModal
          ...
          preview={preview}
          currentPoints={currentQuizPoints}
          setCoins={setCurrentQuizPoints}
          factor={factors}
